/*
 * Based on the https://github.com/mjansson/mdns
 */
#include <stdio.h>
#include <errno.h>
#include <signal.h>
...
service_mdns(const char* hostname, const char* service_name, int service_port) {
	...
	service.txt_record[0] = (mdns_record_t){.name = service.service_instance,
	                                        ...
	                                        .data.txt.key = {MDNS_STRING_CONST("data_flow_control")},
	                                        .data.txt.value = {MDNS_STRING_CONST("software")},
	                                        .rclass = 0,
	                                        .ttl = 0};
	service.txt_record[1] = (mdns_record_t){.name = service.service_instance,
	                                        .type = MDNS_RECORDTYPE_TXT,
	                                        .data.txt.key = {MDNS_STRING_CONST("baud_rate")},
	                                        .data.txt.value = {MDNS_STRING_CONST("115200")},
	                                        .rclass = 0,
	                                        .ttl = 0};
	service.txt_record[2] = (mdns_record_t){.name = service.service_instance,
	                                        .type = MDNS_RECORDTYPE_TXT,
	                                        .data.txt.key = {MDNS_STRING_CONST("radio_type")},
	                                        .data.txt.value = {MDNS_STRING_CONST("ezsp")},
	                                        .rclass = 0,
	                                        .ttl = 0};
...
main(int argc, const char* const* argv) {
	const char* service = "_zigbee-ezsp._tcp.local.";
	const char* hostname = "zigbee-ezsp";
